...
def crear_pesaje(pesaje: PesajeCreate, db: Session = Depends(get_db)):
...
@router.get("/consultar/{cliente_id}", response_model=PesajeRead)
...
def modificar_pesaje(cliente_id: int, pesaje_data: PesajeUpdate, db: Session = Depends(get_db)):
    pesaje = db.query(Pesaje).filter(Pesaje.cliente_id == cliente_id).first()
    if not pesaje:
        raise HTTPException(status_code=404, detail="El pesaje para el cliente ${cliente_id} no existe.")
...
def eliminar_pesaje(cliente_id: int, db: Session = Depends(get_db)):
    pesaje = db.query(Pesaje).filter(Pesaje.cliente_id == cliente_id).first()
    if not pesaje:
