...
from api.schemas.cliente_rutina import ClienteDietaCreate, ClienteDietaRead, ClienteDietaUpdate
from api.db.database import SessionLocal
from api.models.cliente_rutina import ClienteDieta
...
@router.post("/cliente_rutina/asignar", response_model= ClienteDietaRead)
def asignar_rutina(clienteRutina: ClienteDietaCreate, db: Session = Depends(get_db), user=Depends(role_required("entrenador","admin"))):
...
@router.get("/cliente_rutina/{cliente_id}", response_model=ClienteDietaRead)
...
@router.put("/cliente_rutina/{cliente_id}", response_model=ClienteDietaRead)
def actualizar_cliente_rutina(cliente_id: int, cliente_rutina_data: ClienteDietaUpdate, db: Session = Depends(get_db), user=Depends(role_required("entrenador","admin"))):
    cliente_rutina = db.query(ClienteDieta).filter(ClienteDieta.cliente_id == cliente_id).first()
...
@router.delete("/cliente_rutina/{cliente_id}", response_model=ClienteDietaRead)
