...
export function ModifyWindow({ isOpen, ejercicio, onClose, onSave })
{
    const [newValue, setNewValue] = useState(ejercicio);
    ...
    <div className="fixed z-20 inset-0 flex items-center justify-center bg-black/50">
        <div className="bg-white p-4 rounded-lg shadow-lg">
            <div className="form-header max-w-64 mb-6">
                <h2 className="font-bold font-inter text-2xl w-full">Modificar Ejercicio</h2>
            </div>
        <form onSubmit={handleSubmit}>
            ...
            <div className="input flex flex-row w-auto">
                <label htmlFor="form-plan" className="my-auto w-16 font-semibold font-inter text-secondary/80">Grupo Muscular</label>
                <input
                    id="form-plan"
                    type="text"
                    value={newValue.grupo_muscular}
                    onChange={(e) => setNewValue(prev => ({ ...prev, grupo_muscular: e.target.value }))}
                    className="border border-transparent px-2 my-auto border-b-2 border-b-neutral-600 w-fit font-coda focus:outline-0 focus:border-b-primary focus:scale-110 mx-4 transition"
                />
            </div>
            </div>
